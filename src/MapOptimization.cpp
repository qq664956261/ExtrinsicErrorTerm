...
int MapOptimization::readPose(const std::string filepath)
{
    int ret = -1;
...
int MapOptimization::readSonarWaveData(const std::string filepath)
{
    int ret = -1;
...
int MapOptimization::timeStampSynchronization(double sonarWaveTimeStamp)
{
...
int MapOptimization::Sonar2cloud(SonarIndex index, int indexSonar, int indexPose, pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, Eigen::Matrix3d R12, Eigen::Vector3d t12)
{
...
void MapOptimization::ceresAlign()
{
    int eftnum = 0;
...
double MapOptimization::findFirstLapEndTime()
{
    ...
    {
        double distance = std::sqrt((startPose[0] - _Poses[i][1]) * (startPose[0] - _Poses[i][1]) + (startPose[1] - _Poses[i][2]) * (startPose[1] - _Poses[i][2]));
        double timeDiff = _Poses[i][0] * 1e-6 - startTime * 1e-6;
        std::cout << "startPose:" << startPose << std::endl;
        std::cout << "_Poses[i][1]:" << _Poses[i][1] << std::endl;
        std::cout << "_Poses[i][2]:" << _Poses[i][2] << std::endl;
        std::cout << "distance:" << distance << std::endl;
        std::cout << "timeDiff:" << timeDiff << std::fixed << std::endl;
        if (timeDiff > 50 && distance < 0.1)
        {
            _firstLapEndTime = _Poses[i][0] * 1e-6;
            std::cout << "endTime:" << _firstLapEndTime << std::endl;
            break;
        }
    }
    return _firstLapEndTime;
}
void MapOptimization::optimize()
{
    ...
    for (int i = 0; i < _SonarWaveDatas.size(); i++)
    {
        ...
        double sonarTimeStamp = data[0] * 1e-6;
        int index = timeStampSynchronization(sonarTimeStamp);
        if (_Poses[index][0] * 1e-6 < _firstLapEndTime)
            continue;
    ...
    double para_t[_OptimizedPoses.size()][3];
    double para_q[_OptimizedPoses.size()][4];
    double se3[_OptimizedPoses.size()][6];
    for (int i = 0; i < _OptimizedPoses.size(); i++)
    {
        if (_useAutoDiff)
        {
            para_t[i][0] = _OptimizedPoses[i][1];
            para_t[i][1] = _OptimizedPoses[i][2];
            para_t[i][2] = _OptimizedPoses[i][3];
            Eigen::Quaterniond temp_q = Eigen::AngleAxisd(_OptimizedPoses[i][6], Eigen::Vector3d::UnitZ()) *
                                        Eigen::AngleAxisd(_OptimizedPoses[i][5], Eigen::Vector3d::UnitY()) *
                                        Eigen::AngleAxisd(_OptimizedPoses[i][4], Eigen::Vector3d::UnitX());
        ...
        else
        {
            ...
            else
            {
                Eigen::Quaterniond temp_q = Eigen::AngleAxisd(_OptimizedPoses[i][6], Eigen::Vector3d::UnitZ()) *
                                            Eigen::AngleAxisd(_OptimizedPoses[i][5], Eigen::Vector3d::UnitY()) *
                                            Eigen::AngleAxisd(_OptimizedPoses[i][4], Eigen::Vector3d::UnitX());
                Eigen::Vector3d temp_t(_OptimizedPoses[i][1], _OptimizedPoses[i][2], _OptimizedPoses[i][3]);
                Sophus::SE3 SE3_T(temp_q, temp_t);
                Eigen::Matrix<double, 6, 1> se3_T = SE3_T.log();
                se3[i][0] = se3_T[0];
                se3[i][1] = se3_T[1];
    ...
    for (int i = 0; i < _OptimizedPoses.size(); i++)
    {
        ...
        else
        {
            if (_useLineConstraints)
            {
                // problem.AddParameterBlock(para_q, 4, local_parameterization);
                // problem.AddParameterBlock(para_t, 3);
                problem.AddParameterBlock(para_q[i], 4, local_paramso3);
                problem.AddParameterBlock(para_t[i], 3);
            }
            else if (_usePlaneConstraints)
            {
            ...
            {
                problem.AddParameterBlock(se3[i], 6, local_param);
            }
        }
    }
    for (int i = 0; i < _OptimizedPoses.size() - 1; i++)
    {
        Eigen::Vector3d para_t_constraint;
        Eigen::Vector4f para_q_constraint;
        Eigen::Quaterniond temp_q_2 = Eigen::AngleAxisd(_OptimizedPoses[i + 1][6], Eigen::Vector3d::UnitZ()) *
                                      Eigen::AngleAxisd(_OptimizedPoses[i + 1][5], Eigen::Vector3d::UnitY()) *
                                      Eigen::AngleAxisd(_OptimizedPoses[i + 1][4], Eigen::Vector3d::UnitX());
        Eigen::Vector3d temp_t_2(_OptimizedPoses[i + 1][1], _OptimizedPoses[i + 1][2], _OptimizedPoses[i + 1][3]);
        Eigen::Quaterniond temp_q_1 = Eigen::AngleAxisd(_OptimizedPoses[i][6], Eigen::Vector3d::UnitZ()) *
                                      Eigen::AngleAxisd(_OptimizedPoses[i][5], Eigen::Vector3d::UnitY()) *
                                      Eigen::AngleAxisd(_OptimizedPoses[i][4], Eigen::Vector3d::UnitX());
        Eigen::Vector3d temp_t_1(_OptimizedPoses[i][1], _OptimizedPoses[i][2], _OptimizedPoses[i][3]);
    ...
    for (int i = 0; i < _SonarWaveOptimizedDatas.size(); i++)
    {
        std::vector<int> pointSearchInd;
        std::vector<float> pointSearchSqDis;
        float fov_rad = 0.0, sonar_base_x = 0.0, sonar_base_y = 0.0, sonar_base_yaw = 0.0;
        ...
        if (_leftBackBase)
        {
            ...
            sonar_base_y = _leftFronty;
            sonar_base_yaw = _leftFrontyaw;
            length = _SonarWaveOptimizedDatas[i][1];
        }
        else
        {
            fov_rad = _leftBackFovRad;
            ...
            sonar_base_y = _leftBackY;
            sonar_base_yaw = _leftBackYaw;
            length = _SonarWaveOptimizedDatas[i][2];
        }
        ...
        float base_y = sonar_base_y + (sonar_x * sin(sonar_base_yaw) + sonar_y * cos(sonar_base_yaw));
        Eigen::Matrix4d T_wc = Eigen::Matrix4d::Identity();
        Eigen::Quaterniond cur_Q_ = Eigen::AngleAxisd(_OptimizedPoses[i][6], Eigen::Vector3d::UnitZ()) *
                                    Eigen::AngleAxisd(_OptimizedPoses[i][5], Eigen::Vector3d::UnitY()) *
                                    Eigen::AngleAxisd(_OptimizedPoses[i][4], Eigen::Vector3d::UnitX());
        cur_Q_.normalize();
        T_wc.block<3, 3>(0, 0) = cur_Q_.toRotationMatrix();
        T_wc.block<3, 1>(0, 3) = Eigen::Vector3d(_OptimizedPoses[i][1], _OptimizedPoses[i][2], _OptimizedPoses[i][3]);
        Eigen::Vector4d p_ = Eigen::Vector4d(base_x, base_y, 0, 1);
        Eigen::Vector4d p_w = T_wc * p_;
        Eigen::Vector3d p_w_3 = Eigen::Vector3d(p_w[0], p_w[1], p_w[2]);
        pcl::PointXYZI pointSel;
        ...
        if (_leftBackBase)
        {
            ...
            T_base_back.block<3, 3>(0, 0) = q_base_back.toRotationMatrix();
            T_base_back.block<3, 1>(0, 3) = Eigen::Vector3d(_leftBackX, _leftBackY, 0);
            Eigen::Quaterniond q_w_back;
            Eigen::Vector3d t_w_back;
            q_w_back.setIdentity();
            t_w_back.setZero();
            ceres::CostFunction *cost_function = sonarEdgeFactor::Create(
                ...
                q_w_back,
                t_w_back);
            problem.AddResidualBlock(cost_function, NULL, para_q[i], para_t[i]);
        }
        else
        {
            Eigen::Matrix4d T_base_front;
            ...
            Eigen::Matrix4d T_w_base = T_wc;
            Eigen::Matrix4d T_w_front;
            Eigen::Quaterniond q_w_front;
            Eigen::Vector3d t_w_front;
            q_w_front.setIdentity();
            t_w_front.setZero();
            if (_useAutoDiff)
            {
                ceres::LossFunction *loss_function = new ceres::HuberLoss(0.1);
            ...
            else
            {
                if (_useLineConstraints)
                {
                    if (pointSearchSqDis[4] < 1.0)
                    {
                        ...
                        // note Eigen library sort eigenvalues in increasing order
                        Eigen::Vector3d unit_direction = saes.eigenvectors().col(2);
                        Eigen::Vector3d curr_point(base_x, base_y, 0);
                        // std::cout << "saes.eigenvalues():" << saes.eigenvalues() << std::endl;
                ...
                else
                {
                    ...
                    sonarFactor *cost_function = new sonarFactor(Eigen::Vector3d(base_x, base_y, 0),
                                                                 Eigen::Vector3d(_leftFrontCloud->points[pointSearchInd[0]].x, _leftFrontCloud->points[pointSearchInd[0]].y, _leftFrontCloud->points[pointSearchInd[0]].z),
                                                                 q_w_front,
                                                                 t_w_front);
                    problem.AddResidualBlock(cost_function, NULL, se3[i]);
                }
            }
        }
    }
    ...
    for (int i = 0; i < _OptimizedPoses.size(); i++)
    {
        ...
        Eigen::Vector3d t(para_t[i][0], para_t[i][1], para_t[i][2]);
        Eigen::Vector3d p(base_x, base_y, 0);
        p = q * p + t;
    ...
    for (int i = 0; i < _SonarWaveDatas.size(); i++)
    {
        ...
        int index = timeStampSynchronization(sonarTimeStamp);
        if (_leftBackBase)
            Sonar2cloud(SonarIndex::left_back, i, index, resultCloud);
        else
    ...
    for(int i = 0; i < resultCloud->points.size(); i++)
    {
        ...
        point.y = resultCloud->points[i].y;
        point.z = resultCloud->points[i].z;
        point.r = 255;
        point.g = 0;
        point.b = 0;
        _optimizedCloud->points.push_back(point);
    }
    _optimizedCloud->height = 1;
    _optimizedCloud->width = _optimizedCloud->points.size();
    if (_CloudAll->points.size() != 0)
    {
        pcl::io::savePLYFileBinary("_optimizedCloud.ply", *_optimizedCloud);
    }
}