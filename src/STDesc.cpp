#include "ExtrinsicErrorTerm/STDesc.h"
...
// 函数的主要步骤是：
// 遍历体素映射voxel_map的每个元素。
// 检查当前体素是否表示一个平面（is_plane_属性）。
// 对每个体素，检查周围六个方向的邻居（上下左右前后）。
// 计算当前体素与它的邻居之间的法线差异（normal_diff）和法线和（normal_add）。
// 如果法线差异或法线和小于设定的阈值（config_setting_.plane_merge_normal_thre_），则表示体素应连接。
// 相应地更新is_check_connect_和connect_布尔数组，这些数组分别用于标记是否检查了连接和体素是否相连。
// 同时，更新connect_tree_数组，它保存了指向连接的邻居体素的指针。
// OctoTree对象中的 plane_ptr_ 属性是指向一个平面对象的指针，表明体素代表着一个潜在的平面。这个平面对象拥有法线（normal）、中心（center）和其他描述平面的属性。
          ...
          if (!current_octo->is_check_connect_[i]) {
            ...
            if (near_octo->plane_ptr_->is_plane_) {
              ...
              Eigen::Vector3d normal_add = current_octo->plane_ptr_->normal_ +
                                           near_octo->plane_ptr_->normal_;
              std::cout<<"normal_diff.norm():"<<normal_diff.norm()<<std::endl;
              std::cout<<"normal_add.norm():"<<normal_add.norm()<<std::endl;
              std::cout<<"config_setting_.plane_merge_normal_thre_:"<<config_setting_.plane_merge_normal_thre_<<std::endl;
              if (normal_diff.norm() <
...
// STDescManager::corner_extractor 函数的目的是在一个由点云构成的环境中，通过一系列几何操作来提取角点特征。这个过程涉及到对三维空间中的点云进行分析，以找到那些可以代表环境角落或边缘的点。下面是此方法步骤的详细解释：
// 准备阶段：首先，函数创建一个空的 pcl::PointCloud<pcl::PointXYZINormal>::Ptr 对象来保存准备提取的角点。
// 遍历体素地图：函数遍历传入的体素地图 voxel_map。每个体素存储在一个 OctoTree 对象中，代表了一小块三维空间。
// 确定相邻体素：对于每个体素，如果它不构成一个平面（即 plane_ptr_->is_plane_ 为 false），则检查其周围的体素（定义为voxel_round）。该步骤旨在识别那些与当前体素相邻但本身不构成平面的体素。
// 点云投影和角点提取：如果当前体素周围有满足条件的相邻体素，且当前体素包含的点数大于10个，则将当前体素及其周围体素的点投影到一个邻近的平面上。投影使用的平面由相邻体素的平面参数（法线向量和中心点）确定。
// 对投影点进行处理：通过辅助函数 extract_corner（不在提供的代码段中），对投影的点云数据进行处理，以提取表示角点的特征点。处理后的角点会被添加到 prepare_corner_points 列表中，以便之后进行筛选。
// 非极大值抑制：通过 non_maxi_suppression 函数对提取出的角点进行非极大值抑制，目的是消除附近重复或过度密集的角点，从而保留最显著的角点特征。
// 控制角点数量：如果提取的角点数量超过了配置中设定的最大角点数量 config_setting_.maximum_corner_num_，则选择一部分最显著的角点。这通过比较每个角点的强度（或其他度量标准）来完成。
...
// STDescManager::extract_corner函数的作用是从一组投影到平面上的点中提取出角点特征。这在计算机视觉和机器人领域常用于特征提取、三维重建以及SLAM（同时定位与地图构建）等任务中。
// 这个函数主要流程如下：
// 准备投影平面：函数首先确定投影平面，使用给定的proj_normal（法线向量）和proj_center（平面中心点）来定义平面。然后将3D点投影到这个平面上，得到3D点的2D表示。
// 基于距离过滤点：函数根据点与平面的距离将一些点过滤掉，丢弃距离平面太近或太远的点，距离的阈值由dis_threshold_min和dis_threshold_max指定。
// 确定2D位置：对于每个在阈值范围内的3D点，函数计算其在投影平面上的2D位置，这是通过投影到相对于平面法线向量定义的X轴和Y轴来完成的。
// 基于固定分辨率的分割：函数根据指定的resolution将投影的2D点分割成网格，并计算每个网格单元内的点数。
// 计算梯度：函数为2D网格中的每个单元格计算梯度，梯度是指相邻单元格中点数变化的度量。
// 提取角点：梯度最高的单元格（大于配置中预定义的阈值corner_thre_）被认为是角点候选。
// 角点重新投影：然后将这些角点候选重新投影回3D空间，得到原始角点位置。
// 线过滤：执行一次线检查，过滤掉那些可能位于边缘而非真正角点的点。通过线检查和梯度检查的点被添加到corner_points中。
void STDescManager::extract_corner(
    ...
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr &corner_points) {
  std::cout<<"extract_corner"<<std::endl;
...
// 具体步骤如下：
// 初始化协方差矩阵、中心点、法线和用于表示平面的其他数据结构为零。
// 计算所有点云数据的协方差矩阵和几何中心点（代表所有点云数据的平均位置）。
// 对协方差矩阵应用 Eigen::EigenSolver，计算其特征值和对应的特征向量。该过程用于确定主成分，特征向量对应于数据分布的主方向。
// 这里认为最小的特征值对应的特征向量表示平面的法线。这是因为在协方差矩阵中，最小特征值的方向上的点云数据变化最小，根据PCA的几何意义，这个方向垂直于数据分布的平面，故可作为法线。
// 根据法线计算平面模型的截距（intercept_），并更新其他相关属性如is_plane_。
// 如果最小特征值小于设定的阈值（config_setting_.plane_detection_thre_），则认为检测到了一个几何平面，并设置 is_plane_ 属性为 true。
void OctoTree::init_plane() {
  ...
  if (evalsReal(evalsMin) < config_setting_.plane_detection_thre_) {
    ...
    std::cout<<"normal:"<<plane_ptr_->normal_<<std::endl;
    ...
    std::cout<<"is_plane:"<<plane_ptr_->is_plane_<<std::endl;
  } else {
...
void OctoTree::init_octo_tree() {
  std::cout << "voxel points size:" << voxel_points_.size() << std::endl;
